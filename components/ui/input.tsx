...
// ---------------------------------------------------------------------------
//
// Port do input.tsx enviado como referência (preetsuthar17/input) —
// dependência direta do phone-input.tsx. Únicas trocas: "rounded-ele" ->
// "rounded-md" e "shadow-sm/2" -> "shadow-sm" (tokens que não existem
// neste projeto). Resto é fiel ao original.
...
const Input = React.forwardRef<HTMLInputElement, InputProps>(
  ({ className, variant, size, type = "text", leftIcon, rightIcon, error, clearable, onClear, value, ...props }, ref) => {
    ...
    return (
      <div className="relative w-full">
        {leftIcon && (
          <div className="absolute left-3 top-1/2 z-10 -translate-y-1/2 text-muted-foreground [&_svg]:size-4 [&_svg]:shrink-0">{leftIcon}</div>
        )}
